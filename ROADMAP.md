# LF Kitchen - Roadmap

Rencana pengembangan fitur aplikasi LF Kitchen.

---

## 🎯 Versi Saat Ini: v1.1.2

Fitur utama yang sudah tersedia:
- ✅ Manajemen Pesanan (PO)
- ✅ Kasir / POS
- ✅ Manajemen Produk
- ✅ Manajemen Pelanggan
- ✅ Pembelian Bahan Baku
- ✅ Dashboard
- ✅ Produksi Harian
- ✅ Snack Box & Paketan
- ✅ Kwitansi PDF (Desktop & Mobile/Web)
- ✅ Mobile Navigation (Bottom Bar)
- ✅ Dark Mode Toggle
- ✅ Ongkos Kirim (Shipping Cost)
- ✅ Info Pembayaran di Laporan

---

## 📋 Roadmap

### v1.1.0 - Snack Box & Paket (Released)
- [x] Fitur Snack Box dengan isi pilihan
- [x] Paket produk bundling
- [x] Ukuran produk di semua halaman
- [ ] Template box untuk repeat order

### v1.2.0 - Laporan & Analitik
- [ ] Laporan penjualan harian/bulanan
- [ ] Laporan produk terlaris
- [ ] Laporan pembelian bahan
- [ ] Grafik tren penjualan
- [ ] Export ke Excel/PDF

### v1.3.0 - Notifikasi & Reminder
- [ ] Notifikasi pesanan baru
- [ ] Reminder tanggal ambil pesanan
- [ ] Alert stok bahan menipis
- [ ] Notifikasi pembayaran jatuh tempo

### v1.4.0 - Multi-User & Akses
- [ ] Login user
- [ ] Role management (Admin, Kasir, Produksi)
- [ ] Log aktivitas user

### v1.5.0 - Integrasi
- [x] Cetak struk/invoice
- [ ] WhatsApp notification
- [ ] Backup & restore data

---

## 💡 Ide Fitur Masa Depan
- Resep produksi dengan kalkulasi bahan
- Kalender produksi
- Preorder dengan deposit otomatis
- Loyalty program pelanggan
- Katalog online untuk pelanggan
